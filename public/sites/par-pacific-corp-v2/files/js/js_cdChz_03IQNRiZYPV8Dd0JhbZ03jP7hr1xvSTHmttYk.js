...
(function($, Drupal) {
      ...
      $(".alert-submit").once().click(function() {
          var iframe_url = $(".sg-redirect").val();
...
init();
    ...
    var fin_val = $jq(".finidcurrent");
    var fin_detail = fin_val.data("financecurrent"); 
    var row_count = $jq(".rowcount").data("rowcount"); 

    for (var key in fin_detail) {

        var fin_title = fin_detail[key].fin_title;
        var fin_description = fin_detail[key].fin_description.replace(/[^a-zA-ZÀ-ÿ0-9-\/\\^$*+?.,()|[\]{}_ ][']/g, "\\n");
        var fin_address = fin_detail[key].fin_address;
        var start_date = fin_detail[key].start_date;
        var end_date = fin_detail[key].end_date;
        var finance_nid = fin_detail[key].finance_nid;
        var row_count_arc = fin_detail[key].row_count_arc;
        var all_day_event = fin_detail[key].all_day_event;
        if(all_day_event == 'true'){
          var plusDay = new Date(start_date);
          var StartDay = new Date(start_date);
          var EndDay = new Date(end_date);
          var AmPm = format_time(StartDay);
          
        }
        var myCalendar = createCalendar({
         options: {
           class: 'my-class-'+finance_nid
         },
         data: {
    ...
    document.querySelector('#fin-calendarLink-'+finance_nid).appendChild(myCalendar);
     outlookLink = $jq('.icon-outlook').attr("href");
     $jq("#fin-calendarLink-"+finance_nid).attr("href", outlookLink);
     
     $jq(".add-to-calendar").hide();

              var x = $jq("a[class^='icon-outlook']")[key];
              var c = $jq(x).attr("href");
              var y = $jq("a[id^='fin-calendarLink']")[key];
              var z = $jq(y).attr("href", c);
    }

    $jq("li[role='presentation'], .pager__item a").click(function(){

    var row_count_arc = $jq(".rowcountarc").data("rowcountarc"); 
    
    for(var i = 0; i < row_count_arc; i++ ){
              var x = $jq("#profile")[0];
              var t = $jq(x).find("a[class^='icon-outlook']")[i];
              var c = $jq(t).attr("href");
              var y = $jq("a[class^='fin-calendarLink fin-calendarLinkArc-']")[i];
              var z = $jq(y).attr("href", c);
           }
        });

...
jQuery(document).ready(function($){
    $('#headerwrapper #solrstrap-searchbox-form').keyup(keyuphandler);   
...
//jquery plugin allows autoloading of next results when scrolling.
    $.fn.loadSolrResultsWhenVisible = function (q, fq, offset) {
        ...
        $(window).scroll(function (event) {
            if (isScrolledIntoView(elem) && !$(elem).attr('loaded')) {
                ...
                if ($('#headerwrapper #solrstrap-searchbox-form').val() != q) {
                    handle_submit();
                }
    ...
    function handle_submit(event) {
        var q = $.trim($('#headerwrapper #solrstrap-searchbox-form').val());
...
function AutocompleteText() {
    var searchapiurl = base_url+'/searchautocomplete';
        $("#headerwrapper #solrstrap-searchbox-form").autocomplete({
        ...
        select: function (event, ui) {
            ...
            $("#headerwrapper #solrstrap-searchbox-form").val(ui.item.label);    
...
function buildAutocompleParams() {
    var ret = {         
        'language': LANGUAGE,
        "facet_prefix": $.trim($('#headerwrapper #solrstrap-searchbox-form').val())
    };
...
(function ($,drupalSettings) {
    ...
    function getUrlParameter(sParam) {
                ...
                for (i = 0; i < sURLVariables.length; i++) {
                    ...
                    if (sParameterName[0] === sParam) {
                        return sParameterName[1] === undefined ? true : stripHTML(sParameterName[1]);
                    }
                }
        };
    ...
    /*function getUrlParameter(name) {
       ...
       return results === null ? '' : decodeURIComponent(results[1].replace(/\+/g, ' '));
        ...
        var pagecount  = getUrlParameter('page');
            ...
            $("#sortlist").change(function (){
             ...
             if((LANGUAGE!==undefined && LANGUAGE!==null  && LANGUAGE!=='en')) {
                ...
                var url = base_url+'/search-result/?q='+q+'&fq='+nfq+'&querytype='+querytype+'&sort='+selectedtext;
                    ...
                    //var url = base_url+'/search-result/?q='+q+'&fq='+nfq+'&querytype='+querytype+'&sort='+selectedtext; // get selected value
        ...
        /*Meta section Provide define OTP tools  configure  results available START.*/                    
            ...
            if(result.facet_counts.facet_fields.solrdocumenttype){
                ...
                if (result.response.docs.length > 0 ){
                        ...
                        query = query.replace(/\+/g, " ");
                                        if(query) {
                                            ...
                                            $("#result-summary-template").append('<div class="search-result-count">'+textenthere+' '+totalresults+' '+textenresult+'<strong><em>'+query+'</em></strong></div>');
                                ...
                                PAGESIZE = Math.ceil(result.response.numFound / HITSPERPAGE);
                                    ...
                                    * * * * * * * * * * * * * * * * */
                                    ...
                                    {
                                        pageNumber = typeof(pagecount) == "undefined" ? 1 : pagecount;
                ...
                for (var i = 0; i < result.response.docs.length; i++){                      
                    ...
                    //var metathumbimage2 = result.response.docs[i][HITMETATHUMNAILIMAGE];                 
                ...
                if (link.indexOf(".pdf") > -1){
                    var filedocument = "<img src='"+base_url+"/modules/custom/mid_solr_search/img/pdf.jpg'/>";
                }
                if (link.indexOf(".xls") > -1) {
                    var filedocument = "<img src='"+base_url+"/modules/custom/mid_solr_search/img/xls.jpg'/>";
                }
                if (link.indexOf(".doc") > -1) {
                    var filedocument = "<img src='"+base_url+"/modules/custom/mid_solr_search/img/doc.jpg'/>";
                }
                if (link.indexOf(".ppt") > -1) {
                    var filedocument = "<img src='"+base_url+"/modules/custom/mid_solr_search/img/ppt.jpg'/>";
                }
    
     $("#paginate").append('<li class="hittemplate-result" style="list-style: none;"><div class="solrhittemplate"><div class="main-title"><div class="title"><a target="_blank" href="'+hit_data['link']+'">'+hit_data['title']+'</a></div><div class="link">'+hit_data['link']+'<div></div></div><div class="solrhittemplate-content"><span>'+hit_data['text']+'</span></div></div></li>');
    
    //            $("#paginate").append('<li class="hittemplate-result" style="list-style: none;"><div class="solrhittemplate"><div class="main-title"><div class="title">'+hit_data['title']+'</div><div class="link"><a target="_blank" href="'+hit_data['link']+'">'+hit_data['link']+'</a><div></div></div><div class="solrhittemplate-content"><span>'+hit_data['text']+'</span></div></div></li>');
                // $("#paginate").append('<li class="hittemplate-result" style="list-style: none;"><p class="solrhittemplate"><span class="main-title"><a target="_blank" href='+hit_data['link']+'><strong>'+hit_data['title']+'</strong></a></span><span class="solrhittemplatedate">'+hit_data[HITMETAPUBLISHDATE]+'<span id="cap">'+(hit_data[HITMETACONTENTTYPE] ?'['+hit_data[HITMETACONTENTTYPE]+']' : '')+(hit_data[HITOTPTOOLNAME] ?'['+hit_data[HITOTPTOOLNAME]+']' : '')+'</Span></Span></p><p class="solrhittemplate"><span class="solrhittemplate-image">'+metathumbimage+'</span><span class="solrhittemplate-content"><span >'+hit_data['text']+((link.indexOf(".pdf") > -1)?"<img src='"+base_url+"/modules/mid_solr_search/img/pdf.jpg'/>":'')+'</span></p>'/*((filedocument)? filedocument:'')*/+'</p></p><p class="solrhittemplate">'+metatitle+'<span class="video-content">'+metatexbox+'</span></p>'/*<p class="solrhittemplate">'+metavideourl+'<span class="video-content">'+metafile+'</span></p>*/'</li>');
                ...
                $("#solrstrap-searchbox").val(query).focus();
                            //strapline that tells you how many hits you got
                            $("#result-summary-template").append('</br><div class="search-result-count">'+textenthere+' '+totalresults+' '+textenresult+'<strong><i> '+query+' </i></strong></div>');
                            var result_summary_template = $("#result-summary-template").html();
                            $("#result-summary-template").show();
                            if(result_summary_template){
                                Handlebars.compile(result_summary_template);
                    ...
                    //                 //Do not add section metatag filters in chosen filters
                    //                 fqobjs.push({
                    //                     'name': m[1],
                    //                     'value': m[2],
                    //                     'displayvalue': " : " + m[2]
                    ...
                    //             }
                    //             //get OTP facets
                    //             var selectedtab = m[2].replace(/([\\\"])/g, "");
                    //             if (m[1] === TAB_METATAG && ($.inArray(selectedtab, OTPTOOLS) > -1)) {
                    //                 for (var pivotfields in result.facet_counts.facet_pivot) {
                    //                     if (result.facet_counts.facet_pivot[pivotfields].length > 0) {
                    //                         for (var tool in result.facet_counts.facet_pivot[pivotfields]) {
                    //                             for (var pivotentity in result.facet_counts.facet_pivot[pivotfields][tool]['pivot']) {
                    ...
                    //                                     otptoolcount: result.facet_counts.facet_pivot[pivotfields][tool].count,
                    //                                     otptoolname: result.facet_counts.facet_pivot[pivotfields][tool].value,
                    //                                     navs: OTPCAT
                    //                                 }));
                    //                         }
                    //                     }
                    ...
                    success: function(data){  
                        ...
                        $.each(data, function( index, value ){                                                      
                                ...
                                if(q!=null){
                                    if(value.indexOf('(0)') > -1){
                                        $("#solrstrap-tabs").append('<a class="'+active+'">'+value+'</a> ' );
                      ...
                      if((LANGUAGE!==undefined && LANGUAGE!==null  && LANGUAGE!=='en')) {
                        ...
                        var resall = 'Tout';
                                        ...
                                        $("#solrstrap-tabs").append('<a class="'+active+'" href="'+base_url+'/'+LANGUAGE+'/search-result?q='+q+'&querytype='+querytype+'&start=0&page=1'+'">'+value+'</a> ');
                                      }else{
                       $("#solrstrap-tabs").append('<a class="'+active+'" href="'+base_url+'/search-result?q='+q+'&querytype='+querytype+'&start=0&page=1'+'">'+value+'</a> ');
                         ...
                         if((LANGUAGE!==undefined && LANGUAGE!==null  && LANGUAGE!=='en')) {
                                        $("#solrstrap-tabs").append('<a class="'+active+'" href="'+base_url+'/'+LANGUAGE+'/search-result?q='+q+'&ftype=File'+'&querytype='+querytype+'&start=0&page=1'+'">'+value+'</a> ');
                      }else{
                        $("#solrstrap-tabs").append('<a class="'+active+'" href="'+base_url+'/search-result?q='+q+'&ftype=File'+'&querytype='+querytype+'&start=0&page=1'+'">'+value+'</a> ');	
                    }
                        ...
                        /*if(index==='tout'){
                            ...
                            //$("#solrstrap-tabs").prepend('<span style="margin-right:15px;">Filter results by:</span>');
    ...
    */
        ...
        if (ftype !==undefined) {
                ...
                $.ajax({
                    ...
                    success: function(data){
                            ...
                            //strapline that tells you how many hits you got
                            $("#result-summary-template1").append('<div class="search-result-count">'+textenthere+' '+totalresults+' '+textenresult+'<strong><i> '+query+' </i></strong></div>');
                    ...
                    if (result.response.docs.length > 0) {
                        ...
                        for (var i = 0; i < result.response.docs.length; i++) {
                            ...
                            $("#solrstrap-documentfacetresult").append('<li class="hittemplate-result" style="list-style: none;"><div class="solrhittemplate">'+((link.indexOf(".pdf") > -1)?"<img src='"+base_url+"/modules/custom/mid_solr_search/img/search-pdf-icon.png'/>":'')+'<span class="main-title"><div class="title">'+hit_data['title']+'</div><div class="link"><a target="_blank" href='+hit_data['link']+'>'+hit_data['link']+'</a><div></div><div class="solrhittemplate-content"><span>'+hit_data['text']+'</span></div></li>');
                            // $("#solrstrap-documentfacetresult").append('<li class="hittemplate-result" style="list-style: none;"><p class="solrhittemplate"><span class="main-title"><a target="_blank" href='+hit_data['link']+'><strong>'+hit_data['title']+'</strong></a></span></p><p class="solrhittemplate"><span class="solrhittemplate-image">'+metathumbimage+'</span><span class="solrhittemplate-content"><span ><span class="solrhittemplatedate">'+hit_data[HITMETAPUBLISHDATE]+'</Span>'+hit_data['text']+((link.indexOf(".pdf") > -1)?"<img src='"+base_url+"/modules/mid_solr_search/img/pdf.jpg'/>":'')+'</span></p>'/*((filedocument)? filedocument:'')*/+'</p></p><p class="solrhittemplate">'+metatitle+'<span class="video-content">'+metatexbox+'</span></p><p class="solrhittemplate">'+metavideourl+'<span class="video-content">'+metafile+'</span></p></li>');                        
    ...
    var Pagination = {
        ...
        Extend: function(data) {
            data = data || {};
            Pagination.size = parseInt(data.size);
            Pagination.page = parseInt(data.page);
            Pagination.step = parseInt(data.step);
        },
        ...
        Add: function(s, f) {
            for (var i = s; i < f; i++) {
                if(i==1)
                    Pagination.code += '<a pagevalue = \"'+ 0 * (i-1)  +'\">' + i + '</a>';
                else
                    Pagination.code += '<a pagevalue = \"'+ HITSPERPAGE * (i-1)  +'\">' + i + '</a>';
            }
        },
        ...
        if((LANGUAGE!==undefined && LANGUAGE!==null  && LANGUAGE!=='en')) {
            ...
            if(CURRENTPAGE!==undefined && CURRENTPAGE!==null){
                ...
                inputs = inputs+'&start='+CURRENTPAGE+'&page='+pagenum;
            }
        ...
        Bind: function() {
            ...
            for (var i = 0; i < a.length; i++) {             
                if (+a[i].innerHTML === parseInt(pagecount)) a[i].className = 'current';
        ...
        Create: function(e) {
            ...
            if(start=='0'){
                ...
                prevele = start/HITSPERPAGE;
            }
    
    
    
            if(pagecount==Pagination.size){
                nextele = Pagination.size;
            }else{
                nextele = parseInt(pagecount)+1;
            }
    
    
    
            var html = [
                '<a name="prevpager" class="pager-prev" pagevalue='+prevele+'></a>',
                '<span></span>',  // pagination container
                '<a name="nextpager" class="pager-next" pagevalue='+nextele+'></a>'  //next button
            ];
            ...
            if(parseInt(pagecount)<='1'){
                $('#pagination .pager-prev').addClass("pager-disabled pager-disabled__solar");
            }
            if(parseInt(pagecount)===Pagination.size){
                $('#pagination .pager-next').addClass("pager-disabled pager-disabled__solar");
            }
        },
        ...
        Init: function(e, data) {
            ...
            if (pagecount!==undefined){
                ...
                $('#pagination span > a:first-child').addClass("current");
            }
        }
     };
    ...
    /* Date Filtertation Date */   
      ...
      function ExecuteRangeQuery() {
        ...
        var AUTOCOMPLETEFIELD = "tnt_txt_" + LANGUAGE;
        $(document).ready(function () {
            $j("#submitButtonHeader").click(function (e) {
                if (window.location.href.indexOf('/site-services/search-results') < 0) {
                    document.location.href = "/site-services/search-results#queryType=All&q=" + $j("#solrstrap-searchbox").val();
                }
            });
            ...
            $("#solrstrap-searchbox").autocomplete({
                source: function (request, response) {
                    ...
                    $.ajax({
                        ...
                        error: function (result) {
                            console.log("Error loading response data" + result.responseText);                        
                        }
                    }
                          );
                },
        ...
        if(autocompletetemplate) {
            Handlebars.compile(autocompletetemplate);
            $("#solrstrap-searchbox").show(); 
        }
...
(function ($, Drupal) {
  Drupal.behaviors.we_megamenu_accessibility = {
    attach: function (context) {

      // Detect all clicks on the document
      document.addEventListener("click", function(event) {
        // If user clicks inside the menu
        if (!event.target.closest(".region-we-mega-menu")) {
          closeAllDropdown();
        }
      });
      ...
      function initMegaMenu() {
        ...
        // keyboard stuff
        jQuery(megaMenu).delegate('li a', 'keydown', keyboardHandler);
      }
      ...
      function keyboardHandler(keyVent) {
        ...
        if (which === 39) { // RIGHT
          ...
          } else if (isSecondLevel(target) && jQuery(target).hasClass("dropdown-toggle")) {
            openThirdLevelMenu(target);
          } else {
            // dropdown item
          ...
          if (isThirdLevel(target) && isThirdLevelFirstItem(target)) {
            closeThirdLevelMenu();
          }
        } else if (which === 9) { // TAB
          if (isTopLevel(target) && jQuery(target.parentNode).hasClass('dropdown-menu')) {
            keyVent.preventDefault();
            openDropdown(target);
          } else if (!isTopLevel(target) && isLastDropItem(target)) {
            keyVent.preventDefault();
            ...
            if (nextLi) {
              nextAnchor = jQuery("a", nextLi)[0];
              nextAnchor.focus();
            } else {
              // if this is the last item in the top Level menu
              var nextBlock;
              nextBlock = jQuery(target.closest(".region-we-mega-menu").parentNode).next()[0];
              nextAnchor = jQuery("a", nextBlock)[0];
              nextAnchor.focus();
            }
          } else if (isThirdLevel(target) && isThirdLevelLastItem(target)) {
            closeThirdLevelMenu();
          } else if (isSecondLevel(target) && jQuery(target).hasClass("dropdown-toggle")) {
            keyVent.preventDefault();
            openThirdLevelMenu(target);
...
// determines if the item is a top-level one
      function isTopLevel(item) {
        return jQuery(item).is('#mega-menu > li > a');
      }

// determines if the item has a dropdown
      function hasDropdown(item) {
        return jQuery(item.parentNode).hasClass('dropdown-menu');
      }

// determines if the item is the first in the dropdown
      function isFirstDropItem(item) {
        var drop = jQuery(item).closest('.we-mega-menu-submenu')[0];
        var firstInDrop = jQuery('a', drop)[0];
...
// determines if the item is the last in the dropdown
      function isLastDropItem(item) {
        var drop = jQuery(item).closest('.we-mega-menu-submenu')[0];
        var lastInDrop = jQuery('a', drop).last()[0];
...
// finds the next or prev dropdown item
        ...
        if (adjacentSameCol) {
          ...
          if (adjacentCol) {
            ...
            var adjacentItem = (dir === 'next') ?
              jQuery('li a', adjacentCol)[0] :
              jQuery('li a', adjacentCol).last()[0];
      ...
      function openDropdown(item) {
        jQuery(item.parentNode).addClass('menu-item-active');
      ...
      function closeDropdown(item) {
        var droplet = jQuery(item).closest('.we-mega-menu-submenu')[0];
        var topLevelItem = jQuery(droplet).prev()[0];
        jQuery(topLevelItem.parentNode).removeClass('menu-item-active');
      ...
      function closeAllDropdown() {
        var allDrop = jQuery(".we-mega-menu-submenu");
        allDrop.each(function() {
          var topLevelItem = $( this ).prev()[0];
          jQuery(topLevelItem.parentNode).removeClass('menu-item-active');
...
// determines if the item is a second level
      function isSecondLevel(item) {
        return jQuery(item).is('#mega-menu li .we-mega-menu-col:nth-child(1) li > a');
      }

// determines if the item is a third level
      function isThirdLevel(item) {
        return jQuery(item).is('#mega-menu li .we-mega-menu-col:nth-child(2) li > a');
      }

      function isThirdLevelFirstItem(item) {
        var drop = jQuery(item).closest('ul')[0];
        var firstInDrop = jQuery('a', drop)[0];

        return firstInDrop === item;
      }

      function isThirdLevelLastItem(item) {
        var drop = jQuery(item).closest('ul')[0];
        var lastInDrop = jQuery('a', drop).last()[0];
      ...
      function openThirdLevelMenu(item) {
        jQuery(".we-mega-menu-col:nth-child(1) li a").removeClass("hovered");
        jQuery(".we-mega-menu-col:nth-child(2)").find("ul").remove();
        jQuery(item).addClass("hovered");
        jQuery(jQuery(item).closest("li").find("ul").clone()).appendTo(jQuery(item).closest(".we-mega-menu-col").next());
        jQuery(item).closest(".we-mega-menu-col").next().find(".type-of-block").hide();
        jQuery(".we-mega-menu-col:nth-child(2)").find("ul > li > a").first().focus();
      }

      function closeThirdLevelMenu() {
        jQuery(".we-mega-menu-col:nth-child(1)").find(".hovered").removeClass("hovered").focus();
        jQuery(".we-mega-menu-col:nth-child(2)").find("ul").remove();
        jQuery(".we-mega-menu-col:nth-child(2)").find(".type-of-block").show();
      }


      jQuery(document).ready(initMegaMenu);
...
;
jQuery(document).ready(function($){
		/*top nav search JS starts*/

		$(document).on("click", ".top-nav-search > a", function(){
			if($(".top-nav-share").hasClass("active")){
				closePopup();
				$(".top-nav-share").removeClass("active");
				$(".body-overlay").removeClass("on");
				$(".social-share-overlay").removeClass("on");
				//activate popup
				$(this).parent().toggleClass("active");
				$(".body-overlay").toggleClass("on");
				$(".social-share-overlay").toggleClass("on");
				$("body").addClass("overflow-fixed");
			}
			else if($(".top-nav-search").hasClass("active")){
		...
		$(document).on("click", ".top-nav-search.t2-search .search-bar .social-header .bar-close", function(){		
				$(".top-nav-search").removeClass("active");
				$(".body-overlay").removeClass("on");
				$(".social-share-overlay").removeClass("on");
				$("body").removeClass("overflow-fixed");
	    ...
	    $('.board-of-directors-box').on("click",function(){
            $(this).parents().find('.show-overlay').removeClass('show-overlay');
            $(this).find('.bod-image-overlay').addClass('show-overlay');
        });
...
(function (global, factory) {
    ...
    render: function render() {
      ...
      if (params.type === 'bullets') {
        ...
        for (var i = 0; i < numberOfBullets; i += 1) {
          if (params.renderBullet) {
            ...
            paginationHTML += "<" + (params.bulletElement) + " title='change-slide' class=\"" + (params.bulletClass) + "\"></" + (params.bulletElement) + ">";
          }
        }
      ...
      if (params.type === 'fraction') {
        if (params.renderFraction) {
          ...
          paginationHTML =
            "<span name='pagination-bullet' class=\"" + (params.currentClass) + "\"></span>" +
            ' / ' +
            "<span name='pagination-bullet' class=\"" + (params.totalClass) + "\"></span>";
        }
        $el.html(paginationHTML);
      }
      if (params.type === 'progressbar') {
        if (params.renderProgressbar) {
          ...
          paginationHTML = "<span name='pagination-bullet' class=\"" + (params.progressbarFillClass) + "\"></span>";
        }
        $el.html(paginationHTML);
      }
  ...
  var A11y = {
    ...
    create: function create() {
      ...
      Utils.extend(swiper, {
        a11y: {
          liveRegion: $(("<span name='pagination-bullet' class=\"" + (swiper.params.a11y.notificationClass) + "\" aria-live=\"assertive\" aria-atomic=\"true\"></span>")),
        },
      });
...
jQuery(document).ready(function($){
  ...
  if($(".social-share-icon-wrap .swiper-slide").length<6){
    $(".social-share-icon-wrap").addClass("button-hide");
    $(".swiper-button-prev").addClass("button-hide");
    $(".swiper-button-next").addClass("button-hide");
  }
  if ($(window).width() < 992) {
    $(".swiper-container.t2-social-share").addClass("disabled");
  }

  /*top nav share JS starts*/
  $(document).on("click", ".top-nav-share > a", function(){
    if($(".top-nav-search").hasClass("active")){
      ...
      // social share slider
      /*swiper JS start*/
      var share_swiper = new Swiper('.swiper-container.t2-social-share', {
        slidesPerView: 5,
        autoplay:false,
        loop:false,
        spaceBetween:0,
    ...
    else if($(".top-nav-share").hasClass("active")){
      closePopup();
      $("body").removeClass("overflow-fixed");
      $(".top-nav-share").removeClass("active");
      $(".body-overlay").removeClass("on");
  ...
  $(document).on("click", ".top-nav-share .social-header .bar-close", function(){
    if($(".top-nav-share").hasClass("active")){
      $(".top-nav-share").toggleClass("active");
      $(".body-overlay").toggleClass("on");
      $(".social-share-overlay").toggleClass("on");
      $("body").removeClass("overflow-fixed");
    }
  ...
  $(document).keyup(function(event) {
    //enable keyboard buttons for prev and next
    keyCode(event);
  ...
  function keyCode(event) {
    var x = event.keyCode;
    if(event.keyCode == 37) {
      $(".t2-share.active .t2-share-bar .swiper-button-prev").trigger('click');
    }
    //esc key
    if(event.keyCode == 39) {
      $(".t2-share.active .t2-share-bar .swiper-button-next").trigger('click');
    }
    if(event.keyCode == 27) {
      closePopup();
      $(".top-nav-share").removeClass('active');
  ...
  if ($(window).width() < 767) {
    if($("div.social-header").hasClass("link-added")){
      return;
    }else{
      $("div.social-header").addClass("link-added");
      $( '<a href="javascript:void(0)"><span class="sr-only">share</span></a>' ).prependTo("div.social-header");
    }
  }
  $(document).on("click", ".t2-share .social-header a", function(){
    if ($(window).width() < 768) {
      if($(".t2-share").hasClass("active")){
        $(".t2-share").removeClass("active");
        $(".t2-social-share").slideUp("fast");
      }else{
        $(".t2-share").addClass("active");
        $(".t2-social-share").slideDown("fast");
        $('nav.navbar.navbar-we-mega-menu').animate({scrollTop: '+=120px'}, 600);
      }
    }
  });


  /*top nav share JS ends*/
  ...
  $(window).resize(function(){
    ...
    if ($(window).width() < 992) {
      ...
      $(".swiper-container.t2-social-share").removeClass("disabled");
    }
    if ($(window).width() < 768) {
      $(".top-nav-share").removeClass("active");
      $(".t2-social-share").slideUp("fast");
      //append anchor for share accordian functionality
      if($("div.social-header").hasClass("link-added")){
        return;
      }else{
        $("div.social-header").addClass("link-added");

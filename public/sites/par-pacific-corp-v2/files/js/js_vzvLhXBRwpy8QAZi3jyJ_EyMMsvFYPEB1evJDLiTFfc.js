...
(function ($, Drupal) {
   ...
   */
  Drupal.behaviors.snapdown = {
    attach: function (context) {
      $('[id*="accordion-"]', context).css("opacity", "1");

      /* --------- Deep linking related ----------------- */
      $('.container-fluid .ac.first.active', context).removeClass('active');

      /* changes made for nested deeplinking  starts */
      var url = document.location.toString();
      var hash = url.split('#')[1];
      if (hash === undefined) {
        $('.container-fluid .ac.first', context).addClass('active');
        $('.container-fluid .ac.first.active', context).children('.ac-text').show();
      }
      $('.ac', context).each(function () {
        if ($(this).attr('data-id') === hash) {
          $(this).find('.ac-text').parents().show();
          $(this).find('.ac-text').parents('.ac').addClass('active');
          $(this).children('.ac-text').show();
          if ($(this).find('.ac-text .ac').length) {
            $('.ac-text .ac:not(:first-child)', context).removeClass('active').find('.ac-text').hide();
            $('.ac-text .ac.first.active', context).find('.ac-text').show();
          }

          setTimeout(function () {
            var snapdowntop = $('.ac.active', context).offset().top - $('header').outerHeight();
            if (!($('body').hasClass('user-logged-in'))) {
              $('html,body').animate({
                scrollTop: snapdowntop
              }, 500);
            }
          }, 500);
        }
      ...
      /* changes made for accordion issue starts */
      $('.ac-text', context).hide();

      $('.ac-label', context).click(function (e) {
        e.preventDefault();
        ...
        if ($this.parent().hasClass('active')) {
          ...
          if ($this.parent().hasClass('active')) {
            $this.parent().find('.ac-text').find('.ac.first').removeClass('active');
            $this.parent().find('.ac-text').find('.ac.first .ac-text').hide();
          }
          $this.next().slideUp("slow");
        } else {
          $this.parent().find('.ac-text').removeClass('show');
          $this.parent().addClass('active');
          if ($this.parent().hasClass('active')) {
            $this.parent().find('.ac-text').find('.ac.first').addClass('active');
            $this.parent().find('.ac-text').find('.ac.first.active .ac-text').show();
          }

          $this.next().slideToggle("slow");
          window.location.hash = $(this).closest('.ac').data('id');

          var snapdowntop = $(this).parent().offset().top - $('header').outerHeight();
          if (!($('body').hasClass('user-logged-in'))) {

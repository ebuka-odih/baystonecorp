...
;
jQuery(document).ready(function($){
	 $('label[for="edit-field-people-type-target-id"]').addClass("hide");
	$(document).on('click', '.board-of-directors-box:not(.notrigger)', function(){
		var element = $(this);	
			...
			if($(this).hasClass('active')){
				...
				$('.board-of-directors-box').removeClass("active");
				$(this).toggleClass('active');
			}
			if(element != undefined)
			{
				...
				var subelementId = elementId.substr(elementId.lastIndexOf("_"));			
					
				if($(window).width() <= 767){
					var content = '';
					var subElemId = 'bod-wrapper' + subelementId;
					
					if($('#bod-wrapper'+subelementId).is(":visible"))
					{
						$(this).parent().find("#"+subElemId).slideUp("slow");
						$('.board-of-directors--person_name').removeClass("activecategory");
					}
					else{						
						$('.bod-content-wrapped').slideUp("slow");
						$('.board-of-directors--person_name').removeClass("activecategory");
						$('#'+subElemId).slideDown(600);
						$(this).parent().append($('#'+subElemId));
					}
				}
				else{
					if($('#bod-wrapper'+subelementId).is(":visible"))
					{
						$('.bod-content-wrapped').slideUp(600);			
						$(this).find('.board-of-directors--person_name').removeClass("activecategory");			}
					else
					{
						$('.bod-content-wrapped').slideUp("slow");			
						$('.board-of-directors--person_name').removeClass("activecategory");
						$('#bod-wrapper'+subelementId).slideDown(600);
						$(this).find('.board-of-directors--person_name').addClass("activecategory");
					}
				}
		...
		setTimeout(function(){
			if($(window).width() <= 767){
				$('html,body').animate({scrollTop:($('.t2_bod_wrap >div').eq(clickIndex).offset().top)-80}, 500);
			}else{
				$('html,body').animate({scrollTop:($('.t2_bod_wrap >div').eq(clickIndex).offset().top)-10}, 500);
			}
    ...
    //     var divfactor = 3;
    //     var tmp = 1;
    //     $('.bod-content-wrapped').each(function(){
    //         var subcontainerId = 24;
    ...
    //         if(tmp <= divfactor){
    //             if($('#personContainer'+subcontainerId).find($(this).attr('id')).length <= 0)
    //                 $(this).appendTo($('#personContainer'+subcontainerId));
    //         }
    //         tmp = tmp+1;
    //     });
    // });
	...
	$(document).on('click', '.icon-close', function(){	
		$(this).parents().eq(4).slideUp(600);
		$('.board-of-directors-box').removeClass("active");
		$('.board-of-directors-box').find('.show-overlay').removeClass('show-overlay');
	});	
});
